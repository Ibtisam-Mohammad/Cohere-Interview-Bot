...
if (uploaded_file is not None) and (input_role!=''):
  ...
  # Resume/Behavioral questions
    ...
    response_behaviour_q = co.generate(
      model='command-xlarge-nightly',
      prompt=f'''Following text is my resume:\n"{response_resume.generations[0].text}"\nI am preparing for a behavioral interview for a {ROLE} role. Generate {str(NUM_BEHAVIORAL_QUESTION)} questions from my resume related to my education, work experience, projects:''',
      max_tokens=300,
      temperature=1,
      k=0,
      p=0.75,
